...
public final class TinyBufferedKV: KVReadableWritableStore, KVFlushableStore {
    ...
    private func canonicalKey(for key: KVKey) -> BufferKey {
        switch key {
    ...
    private func tinyKVKey(for bufferKey: BufferKey) -> KVKey {
        switch bufferKey {
        case .string(let value):
            return .string(value)
        case .int(let value):
