...
public extension SwiftStomp{
    ...
    func disconnect(force : Bool = false){
        ...
        if !force{ //< Send disconnect first over STOMP
            self.stompDisconnect() { _ in }
        } else { //< Disconnect socket directly! (Not recommended until you have to do it!)
            handleDisconnect()
        }
    }

    func subscribe(to destination : String, mode : StompAckMode = .auto, headers : [String : String]? = nil, completion: ((StompError?) -> ())? = nil){
        ...
        self.sendFrame(frame: StompFrame(name: .subscribe, headers: headersToSend), completion: completion)
    }

    func unsubscribe(from destination : String, mode : StompAckMode = .auto, headers : [String : String]? = nil, completion: ((StompError?) -> ())? = nil){
        var headersToSend = StompHeaderBuilder
            .add(key: .id, value: destination)
            .get
        ...
        self.sendFrame(frame: StompFrame(name: .unsubscribe, headers: headersToSend), completion: completion)
    }

    func send(body : String, to : String, receiptId : String? = nil, headers : [String : String]? = nil, completion: ((StompError?) -> ())? = nil){
        let headers = prepareHeadersForSend(to: to, receiptId: receiptId, headers: headers)

        self.sendFrame(frame: StompFrame(name: .send, headers: headers, stringBody: body), completion: completion)
    }

    func send(body : Data, to : String, receiptId : String? = nil, headers : [String : String]? = nil, completion: ((StompError?) -> ())? = nil){
        let headers = prepareHeadersForSend(to: to, receiptId: receiptId, headers: headers)

        self.sendFrame(frame: StompFrame(name: .send, headers: headers, dataBody: body), completion: completion)
    }

    func send <T : Encodable> (body : T, to : String, receiptId : String? = nil, headers : [String : String]? = nil, jsonDateEncodingStrategy : JSONEncoder.DateEncodingStrategy = .iso8601, completion: ((StompError?) -> ())? = nil){
        let headers = prepareHeadersForSend(to: to, receiptId: receiptId, headers: headers)

        self.sendFrame(frame: StompFrame(name: .send, headers: headers, encodableBody: body, jsonDateEncodingStrategy: jsonDateEncodingStrategy), completion: completion)
    }

    func ack(messageId : String, transaction : String? = nil, completion: ((StompError?) -> ())? = nil){
        ...
        self.sendFrame(frame: StompFrame(name: .ack, headers: headers), completion: completion)
    }

    func nack(messageId : String, transaction : String? = nil, completion: ((StompError?) -> ())? = nil){
        let headerBuilder = StompHeaderBuilder
            .add(key: .id, value: messageId)
        ...
        self.sendFrame(frame: StompFrame(name: .nack, headers: headers), completion: completion)
    }

    func begin(transactionName : String, completion: ((StompError?) -> ())? = nil){
        let headers = StompHeaderBuilder
            .add(key: .transaction, value: transactionName)
            .get

        self.sendFrame(frame: StompFrame(name: .begin, headers: headers), completion: completion)
    }

    func commit(transactionName : String, completion: ((StompError?) -> ())? = nil){
        let headers = StompHeaderBuilder
            .add(key: .transaction, value: transactionName)
            .get

        self.sendFrame(frame: StompFrame(name: .commit, headers: headers), completion: completion)
    }

    func abort(transactionName : String, completion: ((StompError?) -> ())? = nil){
        let headers = StompHeaderBuilder
            .add(key: .transaction, value: transactionName)
            .get

        self.sendFrame(frame: StompFrame(name: .abort, headers: headers), completion: completion)
    }
    ...
    func ping(data: Data = Data(), completion: (((any Error)?) -> Void)? = nil) {
...
fileprivate extension SwiftStomp{
    func stompConnect(completion: @escaping (StompError?) -> ()){
        ...
        self.sendFrame(frame: StompFrame(name: .connect, headers: headers), completion: completion)
    }

    func stompDisconnect(completion: @escaping (StompError?) -> ()){
        //** Add headers
        let headers = StompHeaderBuilder
            .add(key: .receipt, value: "disconnect/safe")
            .get

        self.sendFrame(frame: StompFrame(name: .disconnect, headers: headers), completion: completion)
    }
    ...
    func processReceivedSocketText(text : String){
        ...
        case .error:
            ...
            callbacksThread.async { [weak self] in
                ...
                self._eventsUpstream.send(.error(error: .fromStomp(description: briefDescription, receiptId: receiptId ?? "null")))
            }
    ...
    func sendFrame(frame : StompFrame<StompRequestFrame>, completion: ((StompError?) -> ())? = nil){
        ...
        webSocketTask.send(.string(rawFrameToSend)) { error in
            if let error = error {
                ...
                completion?(StompError.fromSocket(error: error))
                return
            }
            completion?(nil)
        }
            ...
            self.pingTimer = Timer.scheduledTimer(withTimeInterval: self.pingInterval, repeats: true) { [weak self] _ in
                self?.ping() { error in
                    ...
                    case NSURLErrorCancelled:
                        self.stompLog(type: .socketError, message: "ping cancelled")
                        self.pingErrorOccurCount = 0
                    case NSURLErrorNetworkConnectionLost:
                        self.stompLog(type: .socketError, message: "ping ws connection lost")
                        self.pingErrorOccurCount = 0
                        self.disconnect(force: true)
...
extension SwiftStomp: URLSessionWebSocketDelegate {
    ...
    public func urlSession(_ session: URLSession, task: URLSessionTask, didCompleteWithError error: Error?) {
        ...
        callbacksThread.async { [weak self] in
            ...
            self._eventsUpstream.send(.error(error: .fromSocket(error: error)))
        }
    }
